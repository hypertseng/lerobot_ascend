...
def train(cfg: TrainPipelineConfig):
    ...
    # GradScaler
    if NPU_AVAILABLE and device.type == "npu":
        grad_scaler = torch.npu.amp.GradScaler(enabled=True)
    else:
    ...
    if world_size > 1:
        sampler = DistributedSampler(sampler if sampler else dataset, num_replicas=world_size, rank=rank, shuffle=shuffle)
        shuffle = False
    ...
    for step_idx in range(step, cfg.steps):
        ...
        if is_saving_step and cfg.save_checkpoint and rank == 0:
            ...
            policy = policy.module if hasattr(policy, "module") else policy
            save_checkpoint(
